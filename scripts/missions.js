let hitlist = [{name: `Hobsticles`,id:1,xpReward:10,cashReward:10,
...
let savedHits = JSON.parse(localStorage.getItem('hitlist'));
if (savedHits) {
  hitlist = savedHits;
}
...
function nxMs(){
  for (let i = 0; i < hitlist.length; i++) {
    if (!hitlist[i].complete) {
      nextMission = hitlist[i];
      missionIndex = i;
      break;
    }
  }}
